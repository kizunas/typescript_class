...
class AcountingDepartment extends Department { 
  ...
  private static instance: AcountingDepartment // こうすることでinstanceにはAcountingDepartmentのインスタンスが格納される。
  ...
  private constructor(id: string, private reports: string[]) { // プライベートコンストラクターとすると外部からインスタンスがつくれなくなる。
    super(id, 'Accounting');
    this.lastReport = reports[0];
  }

  static getInstance() {
    if (this.instance) { // このメソッドはstaticメソッド。ここのthisはAcountingDepartmentクラス自体を指すことができる。
      return this.instance; // staticメソッドではない場合はthisはオブジェクトを指す
    }
...
//  AcountingDepartmentクラスのコンストラクターがプライベートコンストラクターになっているのでインスタンスが作れない。
//  こういう場合はstaticメソッドをクラス内で使用する。
